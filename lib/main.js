(function() {
  ...
  makeSections = function(lang, data) {
    var code, docs, formatDoc, line, lines, sections;
    lines = data.split('\n');
    sections = [];
    formatDoc = function(line) {
    ...
    while (lines.length) {
      ...
      if (lang.checkType(lines[lines.length - 1]) === 'multiend') {
        while (lines.length) {
          line = lines.pop();
          docs = formatDoc(line) + docs;
          if (lang.checkType(line) === 'multistart') break;
        }
      } else if (lang.checkType(lines[lines.length - 1]) === 'single') {
        while (lines.length && lang.checkType(lines[lines.length - 1]) !== 'code') {
