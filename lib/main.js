(function() {
  var Language, destination, docco_template, ensureDirectory, exec, file_exists, fs, generateDocumentation, generateReadme, generateSourceHtml, getLanguage, jade, languages, makeSections, marked, optimist, options, parseArgs, path, preProcess, relative_base, trimNewLines, walk, writeFile;
  ...
  parseArgs = function(cb) {
    var ext, langFilter, project_name, roots;
    ...
    return exec("find " + (roots.join(' ')) + " -type f \\( " + (langFilter.join(' -o ')) + " \\)", function(err, stdout) {
      ...
      return cb(sources, options.name, options);
    });
  };
