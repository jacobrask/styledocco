(function() {
  var check_config, cloc, content_template, destination, docco_styles, docco_template, dox, dox_template, ensure_directory, exec, ext, file_exists, fs, generate_content, generate_documentation, generate_readme, generate_source_html, get_language, gravatar, highlight, highlight_end, highlight_start, jade, l, languages, parse, parse_args, parse_markdown, path, relative_base, showdown, spawn, walk, write_file, _, _ref;
  ...
  destination = function(filepath, context) {
    ...
    return ("" + context.config.output + "/") + base_path + path.basename(filepath, path.extname(filepath)) + '.html';
  };
  ...
  check_config = function(context, pkg) {
    ...
    return context.config = _.extend(defaults, pkg.docco_configuration || {});
  };
  parse_args(function(sources, project_name, raw_paths) {
    ...
    return ensure_directory(context.config.output, function() {
      var files, next_file;
      ...
      next_file();
      return generate_content(context, context.config.src);
    });
  });
}).call(this);
